...
/// Purpose: Manages CircleCI pipeline customization with form-based input.
/// Parameters: None
/// Returns: Void
/// Preconditions: None
/// Postconditions:
/// - CircleCI pipeline is customized based on user input
/// - Form validation ensures proper values
/// Throws: Never
/// Complexity: O(1)
/// Used By: PopupWindowController
class CircleCICustomizationViewController: BaseCustomizationViewController {
    ...
    private var resourceClassPopUp: NSPopUpButton!
    private var parallelismField: NSTextField!
    
    // MARK: - Initialization
    
    /// Purpose: Initialize CircleCI customization view with template.
    /// Parameters:
    ///   - template: The CircleCI pipeline template to customize
    /// Returns: Void
    ...
    override func setupFormFields() {
        ...
        addPopUpButton(label: "Resource Class:", popUp: &resourceClassPopUp, options: [
            "medium", "large", "xlarge", "2xlarge", "macos"
        ])
        addTextField(label: "Parallelism:", field: &parallelismField, placeholder: "1")
    ...
    override func loadDefaultValues() {
        ...
        resourceClassPopUp.selectItem(at: 4) // macos
        parallelismField.stringValue = "1"
    ...
    override func generateCustomizedContent() -> String {
        ...
        pipeline = pipeline.replacingOccurrences(of: "__RESOURCE_CLASS__", with: resourceClassPopUp.titleOfSelectedItem ?? "macos")
        pipeline = pipeline.replacingOccurrences(of: "__PARALLELISM__", with: parallelismField.stringValue.isEmpty ? "1" : parallelismField.stringValue)
