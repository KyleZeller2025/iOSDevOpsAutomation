...
/// Purpose: Manages Codemagic pipeline customization with form-based input.
/// Parameters: None
/// Returns: Void
/// Preconditions: None
/// Postconditions:
/// - Codemagic pipeline is customized based on user input
/// - Form validation ensures proper values
/// Throws: Never
/// Complexity: O(1)
/// Used By: PopupWindowController
class CodemagicCustomizationViewController: BaseCustomizationViewController {
    ...
    private var maxBuildDurationField: NSTextField!
    
    // MARK: - Initialization
    
    /// Purpose: Initialize Codemagic customization view with template.
    /// Parameters:
    ///   - template: The Codemagic pipeline template to customize
    /// Returns: Void
    ...
    override func setupFormFields() {
        ...
        addPopUpButton(label: "Machine Type:", popUp: &machineTypePopUp, options: [
            "mac_mini_m1", "mac_mini_m2", "mac_pro", "mac_studio"
        ])
        addTextField(label: "Max Build Duration (min):", field: &maxBuildDurationField, placeholder: "60")
    ...
    override func loadDefaultValues() {
        ...
        machineTypePopUp.selectItem(at: 0) // mac_mini_m1
        maxBuildDurationField.stringValue = "60"
    ...
    override func generateCustomizedContent() -> String {
        ...
        pipeline = pipeline.replacingOccurrences(of: "__MACHINE_TYPE__", with: machineTypePopUp.titleOfSelectedItem ?? "mac_mini_m1")
        pipeline = pipeline.replacingOccurrences(of: "__MAX_BUILD_DURATION__", with: maxBuildDurationField.stringValue.isEmpty ? "60" : maxBuildDurationField.stringValue)
